  ...
  return (
    <div className="md:hidden">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        className="grid h-9 w-9 place-items-center rounded-lg border-[3px] border-pop-night bg-white text-pop-night shadow-pop-sm transition-all hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none focus:outline-none focus:ring-4 focus:ring-pop-pink/40"
      >
        <svg
          viewBox="0 0 24 24"
          className="h-5 w-5"
      ...
      {open && (
        <>
          <button
            ...
            onClick={close}
            className="fixed inset-0 top-16 z-20 bg-pop-night/40"
          />
          <nav
            id="mobile-nav-panel"
            className="fixed inset-x-0 top-16 z-30 border-b-[3px] border-pop-night bg-pop-cream shadow-pop"
          >
            <div className="mx-auto max-w-6xl px-4 py-4 sm:px-6">
              ...
              {isAuthed && userEmail && (
                <p className="mt-4 border-t-[3px] border-pop-night pt-4 text-xs font-medium text-pop-night/70">
  ...
  return (
    <li>
      <Link
        href={href}
        onClick={onClick}
        className="block rounded-xl border-[3px] border-transparent px-3 py-3 font-display text-sm uppercase tracking-wide text-pop-night hover:border-pop-night hover:bg-pop-yellow"
      >
        {children}
      </Link>
    </li>
